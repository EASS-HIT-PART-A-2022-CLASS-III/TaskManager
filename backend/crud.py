...
def is_project_owner(db: Session, user_id: int, project_id: int) -> bool:
    return db.query(Project).filter_by(creator_id = user_id, project_id = project_id).first() is not None

def is_project_manager(db: Session, user_id: int, project_id: int) -> bool:
    return db.query(User).join(project_managers).filter_by(user_id = user_id, project_id = project_id).first() is not None

def is_project_member(db: Session, user_id: int, project_id: int) -> bool:
    return db.query(User).join(project_users).filter_by(user_id = user_id, project_id = project_id).first() is not None
...
def create_task(db: Session,project_id: int, task: TaskCreate, curr_user:UserShow) -> Task:
...
def update_task( db: Session,project_id: int, task_id: int, task_data:TaskUpdate) -> Task | None:

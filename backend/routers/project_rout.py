...
from dependencies import get_current_user_manager, get_current_user
import crud
...
def project_add_user(project_id:int, email_to_add: EmailStr,
    ...
    if not user_to_add:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="user don't exist")
...
def project_add_manager(project_id:int, email_to_add: EmailStr,
                     curr_user:User = Depends(get_current_user_manager), db: Session = Depends(get_db)):
    user_to_add = crud.get_user_by_email(db, email_to_add)
    if not user_to_add:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="user don't exist")
    if (crud.is_project_manager(db, user_to_add.id, project_id)):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="user already manager in project")
    return crud.add_user_to_project(db, user_to_add, project_id)


@router.delete("/{project_id}/delete/user", response_model=ProjectShow)
def project_delete_user(project_id:int, email_to_delete: EmailStr,
                        curr_user:User = Depends(get_current_user_manager),db: Session = Depends(get_db)):
    ...
    if not (crud.is_project_member(db, user_to_delete.id, project_id)):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="user don't exists in project")
    return crud.remove_user_from_project(db, user_to_delete, project_id)


@router.get("/my-projects/", response_model=list[ProjectShow])
def user_projects(curr_user:User = Depends(get_current_user) , db: Session = Depends(get_db)):
